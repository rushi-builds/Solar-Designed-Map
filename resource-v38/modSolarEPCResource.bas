...
'   - v3.8 FIX (Date/Time stamp aata hi nahi tha): asli bug
'     ResourceStampLocalRunTime ke andar tha -
'         Cell.NumberFormat = "dd-mm-yyyy hh:nn:ss"
'     "nn" VBA ke Format$() ka minute-code hai, lekin Excel ke
'     Range.NumberFormat me minutes ka code "mm" hota hai - "nn" INVALID
'     hai aur turant runtime error 1004 deta hai. Wo error value likhne se
'     PEHLE aata tha aur caller tak propagate ho jaata tha, isliye
'     lat/lon/Location to bhar jaate the par Date/Time HAMESHA khali
'     rehta tha. Ab: (1) value PEHLE likhi jaati hai, format BAAD me;
'     (2) format code sahi hai ("dd-mm-yyyy hh:mm:ss"); (3) stamp function
'     poora apne error-handling me sealed hai - stamping ki koi bhi dikkat
'     ab kabhi bhi lat/lon/Location fill ko fail nahi kar sakti.
'     PLUS: stamp ab HAR drawn-site fill event pe try hota hai (khud
'     blank-only hai, isliye kuch overwrite nahi hota) - lat/lon pehle se
'     bhare hue row pe bhi blank Date/Time bhar jaata hai. Column-name
'     match ab fuzzy hai (Alt+Enter / extra space / "Date / Time" sab
'     pakde jaate hain) aur auto-create ka ListColumns.Add bhi safe hai.
'     Nayi one-time macro: SolarEPC_ResourceStampMissingDates -> purani
'     rows jinke lat/lon bhare hain par Date/Time blank hai, unhe abhi ke
'     local time se stamp kar deti hai (blank-only, manual kabhi nahi
'     chhuta).
               ...
               Len(Trim$(CStr(RowRange.Cells(1, cLon).Value2))) = 0 Then
                ...
                If Not Site Is Nothing Then
                    ...
                    ResourceFillLocationCell Tbl, RowRange, _
                        ResourceDecimal(CDbl(Site("CentroidLat")), 8), _
                        ResourceDecimal(CDbl(Site("CentroidLon")), 8)
                    'v3.8: blank-only Date/Time stamp on this manual fill too.
                    ResourceStampLocalRunTime Tbl, RowRange
                    Filled = Filled + 1
                Else
                    Skipped = Skipped + 1
                End If
...
Private Sub ResourceWriteToDatabase(ByVal D As Object)
    ...
    'v3.0/v3.8: exact local run stamp (blank-only; column auto-created once).
...
'   5. v3.8: BLANK Date/Time cell me local din+samay stamp hota hai -
'      HAR fill event pe try hota hai, isliye stamp kabhi miss nahi hota
'
...
'v3.8: DONE-check ab Date/Time blank hone pe bhi row dobara process karta hai
'(ResourceProjectRowStamped), taaki purani adhoori rows ka stamp bhi lag jaye.
...
'v3.8: Date/Time stamp ab HAR call pe try hota hai (function khud blank-only
'hai), sirf "kuch naya likha" wale case me nahi - isliye lat/lon pehle se
'bhare hue row ka blank Date/Time bhi bhar jaata hai. Stamp ka result
'DidText me "stamp" ke roop me dikh jaata hai.
Private Function FillResourceDbForSite(ByVal ProjectID As String, _
    ...
    '3. v3.8: Date/Time stamp - HAMESHA try karo (function khud blank-only,
    '   formula-safe aur error-sealed hai). Pehle ye sirf tab chalta tha jab
    '   isi call me kuch likha gaya ho, aur NumberFormat bug se stamping
    '   waise bhi crash ho jaati thi - isi wajah se Date/Time kabhi nahi aata tha.
    If ResourceStampLocalRunTime(Tbl, Target) Then DidText = DidText & "stamp "

    DidText = Trim$(DidText)
...
Public Sub SolarEPC_DrawnLocationDebug()
    ...
    Else
        ...
        P2 = P2 & "   Date/Time column: " & IIf(cDT > 0, _
             "mil gayi (#" & CStr(cDT) & " """ & Rdb.ListColumns(cDT).Name & """)", _
             "TABLE KE ANDAR NAHIN - header row ke andar add karo (auto-create bhi hota hai)") & vbCrLf
        If cProject = 0 Or cLat = 0 Or cLon = 0 Then
        ...
        ElseIf Len(ProjectID) > 0 And CentroidLatitude <> 0 Then
            ...
            Else
                ...
                RowDT = ""
                If cDT > 0 Then RowDT = Trim$(CStr(Rdb.ListRows(MatchRow).Range.Cells(1, cDT).Text))
                P2 = P2 & "5) Matched row #" & CStr(MatchRow) & ": lat=" & IIf(Len(RowLat) > 0, RowLat, "(blank)") & _
                     "  lon=" & IIf(Len(RowLon) > 0, RowLon, "(blank)") & _
                     "  Location=" & IIf(Len(RowLoc) > 0, RowLoc, "(blank)") & _
                     "  Date/Time=" & IIf(Len(RowDT) > 0, RowDT, "(blank)") & vbCrLf
                If cDT > 0 And Len(RowDT) = 0 Then
                    P2 = P2 & "   >> Date/Time blank hai: agla watcher tick ise stamp kar dega," & vbCrLf & _
                              "      ya abhi SolarEPC_ResourceStampMissingDates chalao." & vbCrLf
                End If
            End If
        End If
    End If
...
'--------------------------------------------------------------------------
' v3.0/v3.8: RESOURCE_DB me ek extra column "Run Date-Time (Local)" / apna
' "Date/Time" column - jis DIN aur TIME (aapke computer ki local ghadi) pe
' row ka pehla fill hua (watcher auto-fill YA NASA import), exact stamp.
' Worker ka "Retrieval Date" UTC fetch-time rakhta hai; ye column aapka
' apna local run-time hai. Column na ho to pehli import par apne aap ban
' jaata hai (table ke end me).
'
' v3.8 FIX: column match ab NORMALISED hai - Alt+Enter (line break),
' non-breaking space, double space, "Date / Time" spacing sab handle hote
' hain. Pehle exact-text match tha, isliye header me chhota sa fark bhi
' column ko "nahi mili" bana deta tha.
'--------------------------------------------------------------------------
Private Function ResourceDateTimeColumn(ByVal Tbl As ListObject) As Long
    ...
    'Pass 1: exact (case-insensitive) match - purana behaviour.
    For i = LBound(Names) To UBound(Names)
        ResourceDateTimeColumn = ResourceColumn(Tbl, CStr(Names(i)))
        If ResourceDateTimeColumn > 0 Then Exit Function
    Next i
    'Pass 2 (v3.8): normalised match - line breaks, NBSP, extra spaces hatao.
    On Error GoTo Failed
    For Each Col In Tbl.ListColumns
        CleanHeader = ResourceHeaderNormalise(CStr(Col.Name))
        For i = LBound(Names) To UBound(Names)
            If StrComp(CleanHeader, ResourceHeaderNormalise(CStr(Names(i))), _
                       vbTextCompare) = 0 Then
                ResourceDateTimeColumn = Col.Index
                Exit Function
            End If
        Next i
    Next Col
    Exit Function
Failed:
    ResourceDateTimeColumn = 0
End Function

'v3.8: header text normalise - Alt+Enter/CR/LF/Tab/NBSP -> space, spaces
'collapse, trim. "Date/`Alt+Enter`Time" aur "Date / Time" dono "date/time"
'jaise compare hote hain (slash ke aas-paas ke spaces bhi hata diye).
Private Function ResourceHeaderNormalise(ByVal TextValue As String) As String
    Dim S As String
    S = TextValue
    S = Replace$(S, vbCrLf, " ")
    S = Replace$(S, vbCr, " ")
    ...
    S = Replace$(S, " -", "-")
    S = Replace$(S, "- ", "-")
    ResourceHeaderNormalise = Trim$(S)
End Function

'v3.8 FIX (YE THA ASLI BUG): BLANK-ONLY local run stamp.
'
'Purane v3.5 code me:
'    Cell.NumberFormat = "dd-mm-yyyy hh:nn:ss"    '<- INVALID: Excel "nn" nahi samajhta
'    Cell.Value2 = Now
'"nn" sirf VBA ke Format$() me minutes hota hai; Range.NumberFormat me
'minutes ka code "mm" hai (Excel context se samajh leta hai ki month hai ya
'minute). Invalid format string pe Excel runtime error 1004 phenkta hai -
'value likhne se PEHLE. Wo error caller (FillResourceDbForSite /
'ResourceWriteToDatabase) ke error handler tak propagate hota tha, isliye
'Date/Time HAMESHA blank rehta tha chahe baaki sab kuch bhar jaye.
'
'Ab: (1) value PEHLE likhi jaati hai - agar formatting kabhi bhi fail ho to
'stamp phir bhi lag jaata hai (Excel default date-time format me dikhega);
'(2) format code sahi hai: "dd-mm-yyyy hh:mm:ss"; (3) har step apne
'On Error Resume Next me sealed hai - stamping kabhi bhi lat/lon/Location
'fill ko fail nahi kar sakti.
'
'Return: True = is call me stamp LIKHA gaya; False = column nahi bani /
'cell pehle se bhara tha / formula cell tha.
Private Function ResourceStampLocalRunTime(ByVal Tbl As ListObject, _
    ByVal Target As Range) As Boolean

    Dim C As Long
    Dim Col As ListColumn
    Dim Cell As Range
    Dim StampTime As Date
    ...
    If C = 0 Then
        ...
        If Not Col Is Nothing Then
            Col.Name = "Run Date-Time (Local)"
            'ListColumns.Add kabhi kabhi header pe CalculatedColumn formula
            'ya DataBodyRange me formula chhod deta hai - saaf karo.
            If Not Col.DataBodyRange Is Nothing Then Col.DataBodyRange.ClearContents
        End If
        C = ResourceDateTimeColumn(Tbl)
    End If
    If C = 0 Then
        ResourceSettingDiag "A21", "DATE/TIME STAMP", "B21", "NO COLUMN (auto-create fail)"
        Exit Function
    End If
    ...
    If Cell.HasFormula Then
        ResourceSettingDiag "A21", "DATE/TIME STAMP", "B21", _
            "FORMULA CELL (row " & CStr(Target.Row) & ")"
        Exit Function
    End If
    If Len(Trim$(CStr(Cell.Value2))) > 0 Then     'blank-only, hamesha
        ResourceSettingDiag "A21", "DATE/TIME STAMP", "B21", _
            "NOT BLANK row " & CStr(Target.Row) & ": value=[" & Left$(CStr(Cell.Text), 40) & "]"
        Exit Function
    End If

    StampTime = Now

    'STEP 1 - value pehle. Ye kabhi format pe depend nahi karta.
    Err.Clear
    Cell.Value2 = StampTime
    If Err.Number <> 0 Then
        ResourceSettingDiag "A21", "DATE/TIME STAMP", "B21", _
            "WRITE FAIL row " & CStr(Target.Row) & ": err " & CStr(Err.Number) & _
            " (sheet protected? cell locked?)"
        Exit Function
    End If

    'STEP 2 - format baad me, aur SAHI code ("mm" = minutes is context me).
    'Agar format fail bhi ho jaye to stamp already cell me hai.
    Err.Clear
    Cell.NumberFormat = "dd-mm-yyyy hh:mm:ss"

    ResourceStampLocalRunTime = True
    ResourceSettingDiag "A21", "DATE/TIME STAMP", "B21", _
        "OK row " & CStr(Target.Row) & " = " & Format$(StampTime, "dd-mm-yyyy hh:nn:ss")
    On Error GoTo 0
End Function

'v3.8: ONE-TIME repair macro. Purani RESOURCE_DB rows jinke lat/lon bhare
'hain (yaani fill ho chuka hai) par Date/Time abhi bhi BLANK hai - kyunki
'purane "nn" bug ke waqt stamp lag hi nahi paya tha - unhe ABHI ke local
'time se stamp kar deta hai. Blank-only: bhari hui / manual / formula
'Date/Time cells kabhi nahi chhute. NASA values, Location, kuch nahi badalta.
Public Sub SolarEPC_ResourceStampMissingDates()
    Dim Tbl As ListObject
    Dim R As Long
    Dim cLat As Long, cLon As Long
    Dim Target As Range
    Dim Stamped As Long, Checked As Long

    On Error GoTo Failed
    Set Tbl = ResourceDbTable()
    If Tbl Is Nothing Then
        MsgBox "The RESOURCE_DB table 'resource_db' was not found.", _
            vbExclamation, "Solar EPC Resource"
        Exit Sub
    End If
    cLat = TableColumn(Tbl, "Latitude (" & ChrW(176) & ")")
    cLon = TableColumn(Tbl, "Longitude (" & ChrW(176) & ")")
    If cLat = 0 Or cLon = 0 Then
        MsgBox "RESOURCE_DB requires the Latitude and Longitude columns.", _
            vbExclamation, "Solar EPC Resource"
        Exit Sub
    End If

    For R = 1 To Tbl.ListRows.Count
        Set Target = Tbl.ListRows(R).Range
        'Sirf wo rows jo sach me fill ho chuki hain (lat+lon dono present).
        If Len(Trim$(CStr(Target.Cells(1, cLat).Value2))) > 0 And _
           Len(Trim$(CStr(Target.Cells(1, cLon).Value2))) > 0 Then
            Checked = Checked + 1
            If ResourceStampLocalRunTime(Tbl, Target) Then Stamped = Stamped + 1
        End If
    Next R

    MsgBox "Date/Time stamp repair complete." & vbCrLf & vbCrLf & _
        "Rows with coordinates : " & CStr(Checked) & vbCrLf & _
        "Blank Date/Time cells stamped now : " & CStr(Stamped) & vbCrLf & vbCrLf & _
        "Note: ye stamp AAJ ka repair time hai, original fill time nahi - " & _
        "wo purane bug ke karan record hi nahi ho paya tha. Aaj ke baad ki " & _
        "har nayi drawing pe stamp apne aap sahi time ke saath lagega. " & _
        "Bhari hui / manual / formula cells kabhi overwrite nahi hote.", _
        vbInformation, "Solar EPC Resource"
    Exit Sub
Failed:
    MsgBox "Date/Time stamp repair failed: " & Err.Description, _
        vbExclamation, "Solar EPC Resource"
End Sub
...
'v3.7/v3.8: watcher ki session-memory self-heal: agar koi site DONE mark hai
'par uski RESOURCE_DB row me lat/lon phir se BLANK hain, YA (v3.8) us row ka
'Date/Time abhi bhi blank hai, to sweep dobara process karta hai -> missing
'stamp bhi lag jaata hai. Reopen ki zaroorat nahi.
        ...
        If StrComp(RowProject, ProjectID, vbTextCompare) = 0 Then
            If Len(Trim$(CStr(Rdb.ListRows(R).Range.Cells(1, cLat).Value2))) > 0 And _
               Len(Trim$(CStr(Rdb.ListRows(R).Range.Cells(1, cLon).Value2))) > 0 Then
                'v3.8: lat/lon bhare hain par Date/Time blank hai -> row ko
                '"filled" MAT maano, taaki sweep stamp laga sake.
                If cDT > 0 Then
                    If Len(Trim$(CStr(Rdb.ListRows(R).Range.Cells(1, cDT).Value2))) = 0 Then
