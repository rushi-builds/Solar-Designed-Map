...
'==========================================================================
'
' EXACT DRAWN-SITE LOCATION (NEW IN v2.2 - OFFLINE, NO WORKER, NO INTERNET)
'   The location you drew on the map exists in two exact forms:
'
'     a) the DRAWN BOUNDARY - every corner you clicked, stored by the map in
'        DRAWING_DATA.autoLWHTbl "Coordinates" as
'            lat,lon;lat,lon;lat,lon;...   (8 decimals)
'
'     b) the EXACT SITE CENTROID - the area-weighted centre of that polygon.
'        This is the authoritative coordinate: it is what the Worker sends to
'        NASA POWER and what is written to RESOURCE_DB Latitude (deg) /
'        Longitude (deg).
'
'   Until v2.1 the centroid could only be obtained from the Worker. v2.2
'   computes it INSIDE EXCEL with the byte-identical algorithm, so the exact
'   drawn location is available with no cloud configuration, no internet, no
'   NASA call and no cost:
'
'     - same Earth radius                  (6378137 m, WGS84 semi-major axis)
'     - same vertex rounding               (8 decimals before any maths)
'     - same duplicate/closing-point rules
'     - same local equirectangular projection + area-weighted centroid
'     - same minimum-area rule             (0.01 m2)
'
'   Verified against k12/worker_k12.js polygonCentroidLocalProjection on
'   5000 generated polygons plus 12 hand-built edge cases (closed rings,
'   repeated vertices, triangles, 40-vertex sites, southern hemisphere,
'   sub-metre sites): identical latitude, longitude and area at 8 / 2
'   decimals in every case, and identical accept/reject behaviour.
'   See location-extract/IMPLEMENTATION_GUIDE.md for the parity harness.
'
'   NOTHING ABOUT THE RESOURCE FLOW CHANGED. SolarEPC_ResourceQueueImportedSite
'   still sends the same polygon to the same Worker endpoint; the Worker still
'   owns the authoritative centroid, the cache key and the NASA request. The
'   local functions are read-only extraction helpers.
'==========================================================================
'
' LOCATION RESOLUTION
...
'       A18/B18  last extracted drawn-site location (v2.2, diagnostic only)
'==========================================================================
'
' PUBLIC MACROS
...
'   SolarEPC_ResourceShowDrawnLocation  v2.2: show the EXACT drawn-site location
'                                       (centroid + boundary + area), offline.
'   SolarEPC_ResourceFillDrawnCentroids v2.2: fill BLANK RESOURCE_DB
'                                       Latitude/Longitude from the drawing.
...
'
' PUBLIC FUNCTIONS (usable from any other module, sheet formula helper or
' the Immediate window - all read-only, all offline):
'   SolarEPC_ExactSiteLocation(ProjectID, CentroidLat, CentroidLon) As Boolean
'   SolarEPC_ExactSiteArea(ProjectID) As Double           'm2 of the drawing
'   SolarEPC_ExactSiteLocationText(ProjectID) As String   'one printable line
'   SolarEPC_ExactSiteBoundary(ProjectID, BoundaryText, PolygonJSON, VertexCount)
'
'   The first two and SolarEPC_ExactSiteBoundary are FULLY OFFLINE - they never
'   reverse geocode. SolarEPC_ExactSiteLocationText also resolves the
'   descriptive "Exact Area, Taluka, District" label, so it may use the
'   network exactly like the existing Location auto-fill does.
'==========================================================================
'
' VERSION HISTORY
'   v2.2    EXACT DRAWN-SITE LOCATION EXTRACTION inside Excel. Port of the
'           Worker area-weighted centroid algorithm (verified identical on
'           5000 polygons + 12 edge cases). New public macros
'           SolarEPC_ResourceShowDrawnLocation and
'           SolarEPC_ResourceFillDrawnCentroids, new public functions
'           SolarEPC_ExactSiteLocation / SolarEPC_ExactSiteArea /
'           SolarEPC_ExactSiteLocationText / SolarEPC_ExactSiteBoundary,
'           new helpers
'           ResourceExactSiteData / ResourceParseSitePolygon /
'           ResourceCentroidLocalProjection / ResourceFindSiteRowByProject /
'           ResourceBoundaryText / ResourceCopyToClipboard.
'           Offline and read-only: no Worker, NASA, cache-key, RESOURCE_DB
'           identity, Location-label or resource-flow change.
