...
Private Function ResourceLocationLabel(ByVal LatitudeText As String, ByVal LongitudeText As String) As String
    ...
    'Watcher ticks stay offline so Excel never shows an hourglass.
    If Not mLabelOnlineOk Then GoTo StoreCache
...
Private Function ResourceProjectTableRow(ByVal Tbl As ListObject, _
    ...
    LastUsed = 0
    For R = 1 To Tbl.ListRows.Count
        If Len(Trim$(CStr(Tbl.ListRows(R).Range.Cells(1, cProject).Value2 & ""))) > 0 Then LastUsed = R
        If Len(Trim$(CStr(Tbl.ListRows(R).Range.Cells(1, cLatitude).Value2 & ""))) > 0 Then LastUsed = R
        If Len(Trim$(CStr(Tbl.ListRows(R).Range.Cells(1, cLongitude).Value2 & ""))) > 0 Then LastUsed = R
    Next R
...
Public Sub SolarEPC_ManualFillNow()
    ...
    mLabelOnlineOk = True
    ResultText = FillResourceDbForSite(ProjectID, Lat, Lon, True, True)
    mLabelOnlineOk = False
